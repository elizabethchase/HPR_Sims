...
write_to_folder = "/home/ecchase/sensitivity_gaussian/";
setwd("/home/ecchase")
...
sig_scale <- sig_list[fullcombs$Var2[rowid]]
n <- fullcombs[rowid, 1]
...
mymodel <- hpr(y = y, X = X, family = "gaussian", intercept_mean = alpha_mean);
...
mymodel <- hpr(y = y, X = X, family = "gaussian", intercept_sd = alpha_sd);
mycurvefits <- get_preds(mymodel, alpha = 0.05)
mycurvefits$x <- X[,1]
mycurvefits$n <- n
...
mymodel <- hpr(y = y, X = X, family = "gaussian", c = c);
mycurvefits <- get_preds(mymodel, alpha = 0.05)
mycurvefits$x <- X[,1]
mycurvefits$n <- n
...
mymodel <- hpr(y = y, X = X, family = "gaussian", sig_scale = sig_scale);
mycurvefits <- get_preds(mymodel, alpha = 0.05)
mycurvefits$x <- X[,1]
mycurvefits$n <- n
mycurvefits$param <- paste0("sig_scale = ", sig_scale)
mycurvefits$truth <- true_curve$truth

mycompperf <- get_diagnostics(mymodel)
mycompperf$n <- n
mycompperf$param <- paste0("sig_scale = ", sig_scale)
