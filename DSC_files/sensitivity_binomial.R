...
write_to_folder = "/home/ecchase/sensitivity_binomial/";
setwd("/home/ecchase")
...
mymodel <- hpr(y = y, X = X, family = "binomial", intercept_mean = alpha_mean);
...
mymodel <- hpr(y = y, X = X, family = "binomial", intercept_sd = alpha_sd);
mycurvefits <- get_preds(mymodel, alpha = 0.05)
mycurvefits$x <- X[,1]
mycurvefits$n <- n
...
mymodel <- hpr(y = y, X = X, family = "binomial", c = c);
mycurvefits <- get_preds(mymodel, alpha = 0.05)
mycurvefits$x <- X[,1]
mycurvefits$n <- n
