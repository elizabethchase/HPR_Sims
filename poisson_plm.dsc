...
Pspline: R( library(gamlss);
            ...
            newdata$x <- X[,1];
            start.time <- Sys.time();
            mymodel <- gamlss(y ~ pb(x, control = pb.control(order = 1)) + Weight + Income + Sex + Height, data = newdata, family = "PO");
            ...
            mypredfits <- data.frame("Median" = exp(preds_ys$fit), "Lower" = exp(preds_ys$fit - 1.96*preds_ys$se.fit), "Upper" = exp(preds_ys$fit + 1.96*preds_ys$se.fit), "x" = X[,1]);
            mycompperf <- data.frame("Time" = time.taken, "Divergences" = NA, "Max_Treedepth" = NA, "RHat" = NA, "Min_Ess_Bulk" = NA, "Min_Ess_Tail" = NA, "Num_Param" = NA, "Num_Samples" = NA))
  X: $preds
  y: $outcome
  Z: $lin_preds
